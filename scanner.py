...
MAX_TICKERS = 1000
BATCH_SIZE = 50
RATE_SLEEP = 1.0 # seconds between requests (increase if hitting rate limits)
...
BREAKOUT_LOOKBACK = 20 # days to compute recent high (exclude today)
...
continue
su = sym.strip().upper()
# basic sanitation: skip tickers with spaces/slashes/dots (optional)
...
if not tickers:
return [], 0, 0
slot = int(time.time() // 300) # 5-minute slot
...
# average volume: previous 30 days (or fewer if not available)
...
# breakout check: price > recent N-day high (exclude today)
high20 = None
...
scan_set = list(dict.fromkeys(batch + list(hot_list)))
